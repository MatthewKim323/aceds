...
### API uncertainty (`predicted_gpa_std` + intervals)

The FastAPI `POST /predict` field `predicted_gpa_std` is the **held-out test residual RMSE** for the section’s cold-start **regime** (same buckets as the table above). Values are fixed in `backend/app/ml/predictor.py` (`regime_residual_rmse`) and match `processed/cold_start_report.md`.

**Symmetric intervals (`gpa_lo`, `gpa_hi`, `interval_half_width`):**

- If `backend/app/ml/artifacts/conformal_quantiles.json` is present (generated by `data_pipeline/scripts/21_conformal_calibration.py` on the **validation** split), half-widths use the **(1−α/2)** empirical quantile of **|y − ŷ|** per regime (split-style calibration on val).
- Otherwise the API uses a **Gaussian fallback**: `1.645 × predicted_gpa_std` (~90% two-sided if residuals were Gaussian — they are not; this is conservative documentation).

`POST /predict` also returns `model_version` (from `model_meta.json`) and `conformal_method` (`split_abs_residual_val` vs `gaussian_fallback`).

### Risk-aware schedule objective

`POST /optimize` accepts `preferences.risk_lambda` (default `0`). The PuLP grade term uses **effective GPA** `clip(μ − λ · interval_half_width, 0, 4)` before the usual 0..1 normalization. Feasibility constraints are unchanged. Toy ablation: [DECISION_EVAL.md](DECISION_EVAL.md).

### Regime reliability figure

After `python data_pipeline/scripts/22_regime_reliability.py`: `data_pipeline/processed/pitch/07_regime_reliability.svg` (binned predicted vs actual means on the **test** split, faceted by regime).
